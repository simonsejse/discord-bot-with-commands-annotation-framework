...
public class BackForwardCommand implements CommandPerform {
    ...
    public void perform(SlashCommandEvent event) throws CommandException {
        ...
        event.deferReply(false).queue(iHook -> {
            iHook.sendMessage(String.format("Sangen `%s` af `%s` blev spolet `%ds` tilbage", trackInfo.title, trackInfo.author, timeInSeconds)).queue();
        });
    }
